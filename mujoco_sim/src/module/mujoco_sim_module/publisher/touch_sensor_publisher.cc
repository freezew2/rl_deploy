...
void TouchSensorRos2Publisher::Initialize(YAML::Node options_node) {
  ...
  AIMRT_CHECK_ERROR_THROW(aimrt::channel::RegisterPublishType<joint_msgs::msg::FootSensorState>(publisher_), "Register touch sensor publish type failed.");
}
...
void TouchSensorRos2Publisher::PublishSensorData() {
  ...
  executor_.Execute([this, state_array = std::move(state_array)]() {
    joint_msgs::msg::FootSensorState state;
    ...
    for (size_t i = 0; i < touch_sensor_group_num_; ++i) {
      ...
      joint_msgs::msg::FootSensor single_state;
      single_state.presure.resize(touch_sensor_num_vec_[i]);

      for (size_t j = 0; j < touch_sensor_num_vec_[i]; ++j) {
        single_state.presure[j] = static_cast<int16_t>(state_array[i].state_vec[j]);
      }
